<?php
	...
	function getRowsFromDB($parent_id){
		...
		GLOBAL $oMysql;	
		$sConditions = " contactos.sEstado = 'A'";
		$array=$oMysql->consultaSel("CALL usp_getContactos(\"$sConditions\",\"\",\"\",\"\");");
		...
		if(count($array)>0)
		{
			foreach ($array as $row){
				...
				print("<row id='".$row['id']."'>");
				  ...
				  print("<cell><![CDATA[".stripslashes($row['sDireccion'])."]]></cell>");
				  print("<cell><![CDATA[".stripslashes($row['sMail'])."]]></cell>");
				  print("<cell><![CDATA[".stripslashes($row['sTelefono'])."]]></cell>");
				  print("<cell><![CDATA[".stripslashes($row['sMovil'])."]]></cell>");
				  print("<cell><![CDATA[".stripslashes($row['sNombreGrupoUsuario'])."]]></cell>");
				print("</row>");			
