...
function updateDatosUser($aDatos){
   ...
   return $oRespuesta;		
	...
	function enviarMailsRapidos($form){
		...
		$mail = new PHPMailer();
		$mail->SetLanguage("es","../includes/clases/phpmailer/language/");
		...
		$mail->Port = 465;//587
		$mail->SMTPAuth = true;
		$mail->Username = "info@grivasoluciones.com.ar";
		$mail->Password = "superinfo";
		$mail->From = "info@grivasoluciones.com.ar";
		$mail->FromName = "Griva Computacion";
		$mail->Subject = "Promociones";
		$mail->AddAddress($form['sMail']);
		$mail->IsHTML(true);
	...
	function enviarMailsMasivos($form){
		...
		$array['Subject'] = "Promociones";	
		$array['Message'] = $form['sMensaje'];
		$array['From'] = "jricardone@gmail.com";
		$cabeceras  = 'MIME-Version: 1.0' . "\r\n";
		$cabeceras .= 'Content-type: text/html; charset=iso-8859-1' . "\r\n";
		//$cabeceras .= 'From: GRIVA Computacion <info@grivasoluciones.com.ar>' . "\r\n";
		$cabeceras .= 'From: GRIVA Computacion <info@grivacomputacion.com.ar>' . "\r\n";
		
		$sMsje = "";
		//$oRespuesta->alert(count($aContactos));
		if(count($aContactos)>0){
			$aMailError = array();
			$iCantidadEnviados = 0;
			
			$set = "idGrupo,idUsuario,idEstadoTrabajo,idTipoTrabajo,sMensaje,dFechaRegistro,sObservacion";			
			$values = "'{$form['idGrupo']}','{$_SESSION['ID_USER']}',1,2,'{$form['sMensaje']}',NOW(),''";
			$ToAuditry = "Envio de E-Mail Masivo ::: Usuario={$_SESSION['ID_USER']} - Grupo={$form["idGrupo"]}";
			$idTrabajo = $oMysql->consultaSel("CALL usp_InsertTable(\"trabajos\",\"$set\",\"$values\",\"{$_SESSION['ID_USER']}\",\"12\",\"$ToAuditry\");",true);	
			...
			foreach($aContactos as $row){
				...
				if($smsError != '_OK_'){
					$aMailError[] = $row['sMail'];
					$sql = "INSERT INTO mensajestrabajos(idTrabajo,idContacto,idEstadoMensaje) VALUES({$idTrabajo},{$row['id']},3)";
				}else{
					$iCantidadEnviados++;
					$sql = "INSERT INTO mensajestrabajos(idTrabajo,idContacto,idEstadoMensaje) VALUES({$idTrabajo},{$row['id']},1)";
				}
				$oMysql->consultaAff($sql);
			}
			
			//$oRespuesta->alert("cant mail error ".count($aMailError));
			//$oRespuesta->alert("cant mail env ".$iCantidadEnviados);
			...
			if($iCantidadEnviados == count($aContactos)){
				$idEstadoTrabajo = 2;
			}else{
				$idEstadoTrabajo = 3;
			}
			//$oRespuesta->alert("estado trabaj ".$idEstadoTrabajo);
			$sql = "UPDATE trabajos SET trabajos.idEstadoTrabajo={$idEstadoTrabajo} WHERE trabajos.id={$idTrabajo}";
			$oMysql->consultaAff($sql);
	...
	function updateDatosModem($form){
		...
		$oMysql->consultaAff("CALL usp_UpdateTable(\"params\",\"$set\",\"$sConditions\",\"{$_SESSION['ID_USER']}\",\"20\",\"$ToAuditry\");");				
